...
class BaseFeeder(data.Dataset):
    ...
    def __getitem__(self, idx):
        if self.data_type == "video":
            ...
            info=self.inputs_list[idx]['original_info']
            s='fullFrame-256x256px/train/' + info.split("|")[0] + '/1/' + info.split("|")[0][0:info.split("|")[0].index('de') - 1]
            # s='fullFrame-256x256px/dev/' + info.split("|")[0] + '/1/' + info.split("|")[0][0:info.split("|")[0].index('de') - 1]
            keypoint=self.get_keypoint(s)
            # input_data, label = self.normalize(input_data, label, fi['fileid'])
            return input_data, torch.LongTensor(label), info,keypoint
    ...
    def get_keypoint(self,name):
        with open('/media/cv3/store1/postgraduate/y2023/WLF/NLA-SLR/keypoints/phtrain.pkl', 'rb') as f:
            ...
            s=name2all_keypoints[name]
        # with open('/media/cv3/store1/postgraduate/y2023/WLF/NLA-SLR/keypoints/phdev256.pkl', 'rb') as f:
        #     name2all_keypoints = pickle.load(f)
        #     s=name2all_keypoints[name]
        return s
    ...
    def collate_fn(batch):
        ...
        if len(video[0].shape) > 3:
            ...
            padded_video = [torch.cat(
                ...
                for vid in video]
            # 关键点填充 - 同步处理
            # 关键点形状为(T, 133, 3)，T为原始帧数
            for kpts in keypoint:
                kpts=torch.from_numpy(kpts)
                padded_keypoint = torch.cat(
                    (
                        kpts[0][None].expand(left_pad, -1, -1),
                        kpts,
                        kpts[-1][None].expand(max_len - len(kpts) - left_pad, -1, -1),
                    ), dim=0)
            padded_keypoints.append(padded_keypoint)
            ...
            for kpts in keypoint:
                kpts=torch.from_numpy(kpts)
                padded_kpts = torch.cat(
                    (
                        kpts,
                        kpts[-1][None].expand(max_len - len(kpts), -1, -1),
                    ),
                    dim=0
                )


            padded_keypoints.append(padded_kpts)
        label_length = torch.LongTensor([len(lab) for lab in label])
        if max(label_length) == 0:
            return padded_video, video_length, [], [], info,padded_keypoints
        else:
            padded_label = []
            for lab in label:
                padded_label.extend(lab)
            padded_label = torch.LongTensor(padded_label)
            return padded_video, video_length, padded_label, label_length, info,padded_keypoints
